...
3) Consultar NCBI para obter tax_id específico de cada 'organism' em df_ct_taxonomy
   - Gera/atualiza df_tax_id.parquet (com retomada e backoff).
"""
...
def build_df_experiments(df_marmags: pl.DataFrame) -> pl.DataFrame:
    ...
    failed_runs = []
    backoff_times = [0, 30, 60, 120, 240]  # segundos
    batch_size = 150

    for batch_idx, batch_runs in enumerate(chunked(runs, batch_size), start=1):
        print(f"\n=== Processando bloco {batch_idx} com {len(batch_runs)} RUNs ===")

        for run_acc in tqdm(batch_runs, desc=f"Bloco {batch_idx}"):
            ...
            for attempt_idx, wait_seconds in enumerate(backoff_times):
                if wait_seconds > 0:
                    print(f"[{run_acc}] Tentativa {attempt_idx+1}, esperando {wait_seconds}s antes...")
                ...
                try:
                    ...
                    success = True
                    time.sleep(0.2)  # pequeno intervalo entre RUNs
                    break
                except Exception as e:
                    last_error = e
                    print(f"[ERRO] RUN {run_acc} falhou na tentativa {attempt_idx+1}: {e}")

            if not success:
                print(f"[FALHA DEFINITIVA] RUN {run_acc} após todas as tentativas. Guardando em failed_runs.")
                failed_runs.append(run_acc)

        # Checkpoint ao fim de cada bloco
        df_experiments.write_parquet("df_experiments.parquet")
        print(f"Checkpoint salvo: df_experiments.parquet com {df_experiments.height} linhas.")

        if failed_runs:
            pl.DataFrame({"run": failed_runs}).write_parquet("failed_runs.parquet")
            print("Lista de falhas parcial salva em failed_runs.parquet.")

        time.sleep(3)  # pausa entre blocos
...
# ---------------------------------------------------
# Parte 3: NCBI – df_tax_id com backoff + retomada
# ---------------------------------------------------
...
def get_taxonomy_id_with_retry(query: str) -> str | None:
    ...
    for attempt_idx, wait_seconds in enumerate(BACKOFF_TIMES_NCBI):
        if wait_seconds > 0:
            print(f"[{query}] Esperando {wait_seconds}s antes da tentativa {attempt_idx+1}...")
            time.sleep(wait_seconds)
        ...
        try:
            ...
            # códigos diferentes de 200 -> tenta de novo até esgotar
            print(f"[{query}] HTTP {response.status_code} na tentativa {attempt_idx+1}.")
        except Exception as e:
            last_error = e
            print(f"[ERRO] Falha ao consultar NCBI para '{query}' tentativa {attempt_idx+1}: {e}")

    print(f"[FALHA DEFINITIVA] '{query}' após todas as tentativas.")
    return None
...
def build_df_tax_id(df_ct_taxonomy: pl.DataFrame) -> pl.DataFrame:
    ...
    com retomada (checkpoint) e backoff.
    """
    ...
    print(f"Falharam definitivamente {len(failed_samples)} queries.")

    if failed_samples:
        pl.DataFrame({"sample_name": failed_samples}).write_parquet("df_tax_id_failed.parquet")
        print("df_tax_id_failed.parquet salvo.")
...
#module load python   # se precisar
#pip install polars pandas requests tqdm  # ou usa conda/venv

#python marmags_metadata_pipeline.py